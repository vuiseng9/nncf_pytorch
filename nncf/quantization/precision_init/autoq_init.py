...
class AutoQPrecisionInitializer(BasePrecisionInitializer):
    ...
    def _dump_best_episode(self, info_tuple: Tuple, bit_stats_df: pd.DataFrame, env: 'QuantizationEnv'):
        if self._dump_autoq_data:
            ...
            list_of_dump_dict = []
            for i, _ in enumerate(env._groups_of_adjacent_quantizers):
    ...
    def _dump_episode(self,
        ...
        if self._dump_autoq_data:
            ...
            list_of_dump_dict = []
            for i, _ in enumerate(env._groups_of_adjacent_quantizers):
                list_of_dump_dict.append(env.master_df.loc[env.adjq_groupwise_df_lut_keys[i], ["action", "action_aligned"]].to_dict())
